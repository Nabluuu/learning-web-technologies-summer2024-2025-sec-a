...
if (!isset($_COOKIE['meal_count'])) {
    setcookie("meal_count", 0, time()+60, "/"); 
...
$success = "";
    ...
    if ($carbs === "" || !is_numeric($carbs)) {
        $errors[] = "Carbs must be numeric.";
    }
    if ($protein === "" || !is_numeric($protein)) {
        $errors[] = "Protein must be numeric.";
    }
    if ($fat === "" || !is_numeric($fat)) {
        $errors[] = "Fat must be numeric.";
    }

    $total = (int)$carbs + (int)$protein + (int)$fat;
    if ($total > 100) {
        $errors[] = "Total macros cannot exceed 100g.";
    }

    if (empty($errors)) {
        if ($_COOKIE['meal_count'] <1000) {
            $new_count = $_COOKIE['meal_count'] + 1;
            setcookie("meal_count", $new_count, time()+3600, "/");
            $_COOKIE['meal_count'] = $new_count;

            $success = "Meal '$name' added successfully! (Total meals this session: ".$_COOKIE['meal_count'].")";
        } else {
            $errors[] = "cookie limit reached.";
        }
    }
...
if (!isset($_SESSION['meals'])) {
    $_SESSION['meals'] = []; // store meals here
}

function validateInput($name, $c, $p, $f) {
    if ($name === "") return false;
    if (!is_numeric($c) || !is_numeric($p) || !is_numeric($f)) return false;
    if ($c + $p + $f > 100) return false;
    return true;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (validateInput($name, $c, $p, $f)) {
        // Add meal to session (acts like your JS "mealList.appendChild")
        $_SESSION['meals'][] = [
            'name' => $name,
            'carbs' => $c,
            'protein' => $p,
...
<head>
  ...
  <style>
    ...
    header { background: #4CAF50; color: white; padding:2rem; text-align: center; }
    nav { display: flex; justify-content: center; background: #333; }
    nav button { flex: 1; padding: 1rem; border: none; background: #333; color: white; cursor: pointer; }
    nav button:hover { background: #555; }
    .navbar { display: flex; gap: 25px; padding: 15px 80px; background: rgba(0, 0, 0, 1); position: fixed; top: 0; width: 100%; z-index: 10; backdrop-filter: blur(4px); }
    ...
    .meal-item { background: white; margin: 5px 0; padding: 10px; border-radius: 5px; display: flex; justify-content: space-between; align-items: center; }
    ...
    .legend span { display: inline-block; margin-right: 15px; cursor: pointer; }
    .color-box { display: inline-block; width: 15px; height: 15px; margin-right: 5px; vertical-align: middle; }
    .error-msg { color: red; font-size: 0.9em; margin-top: 2px; }
  </style>
</head>
<body>
  <header>
  ...
  <div id="diary" class="screen active">
    ...
    <form method="post" action="">
      <input type="text" name="foodName" placeholder="Food Name" value="<?php echo $_REQUEST['foodName'] ?? ''; ?>">
      <input type="number" name="carbs" placeholder="Carbs (g)" value="<?php echo $_REQUEST['carbs'] ?? ''; ?>">
      <input type="number" name="protein" placeholder="Protein (g)" value="<?php echo $_REQUEST['protein'] ?? ''; ?>">
      <input type="number" name="fat" placeholder="Fat (g)" value="<?php echo $_REQUEST['fat'] ?? ''; ?>">
  ...
  <div class="navbar">
    <a href ="Nutrition.php">Nutrition Logger</a>
